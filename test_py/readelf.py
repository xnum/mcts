...
def main(name, method, limit):
    ...
    initial_state = proj.factory.entry_state(args=[name, argv1, "fake"],remove_options={simuvex.s_options.LAZY_SOLVES}) 

    path_group = proj.factory.path_group(initial_state, method=method, limit=limit)
    path_group.run()

if __name__ == '__main__':
    main('/usr/bin/readelf', sys.argv[1], int(sys.argv[2]))
