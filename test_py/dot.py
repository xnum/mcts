...
# ./conv < cp_DFS.txt | dot -T png -o cp_DFS.png
...
def main(name, method, limit):
    ...
    argv4 = angr.claripy.BVS("argv4", 0xE * 20)
    initial_state = proj.factory.entry_state(args=[name, argv1, argv2, argv3, argv4],remove_options={simuvex.s_options.LAZY_SOLVES}) 

    path_group = proj.factory.path_group(initial_state, method=method, limit=limit)
    path_group.run()

if __name__ == '__main__':
    main('/usr/bin/dot', sys.argv[1], int(sys.argv[2]))
