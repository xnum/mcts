...
def main(name, method, limit):
    ...
    argv1 = angr.claripy.BVS("argv1", 0xE * 30)
    initial_state = proj.factory.entry_state(args=[name, argv1, "fake.gif"],remove_options={simuvex.s_options.LAZY_SOLVES}) 

    path_group = proj.factory.path_group(initial_state, method=method, limit=limit)
    path_group.run()

if __name__ == '__main__':
    main('/usr/bin/gif2png', sys.argv[1], int(sys.argv[2]))
